...
to `gnss_timeseries_analysis`, `station_network_analysis`, and may delegate to
`seismic_event_catalog` only when event-context evidence is part of the task, so
each requested analysis stage is an inspectable branch.
