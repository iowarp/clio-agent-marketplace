...
**Related skills**: `hdf5_chunking` for chunk layout (required for compression), `hdf5_io` for write buffering.
...
| `-l` | Set chunks (see `hdf5_chunking`) | `-l /dset:CHUNK=100x100` |
...
8. **Combine with chunking** optimization (see `hdf5_chunking`)
