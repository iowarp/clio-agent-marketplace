...
**Related skills**: `hdf5_chunking` for chunk layout and cache, `hdf5_filters` for compression.
...
Increase cache when making many small updates (see `hdf5_chunking` for details):
...
**Sequential access** (entire dataset): Use contiguous storage (see `hdf5_chunking`), read large blocks.

**Random access** (scattered elements): Use chunked storage aligned with pattern (see `hdf5_chunking`), increase chunk cache.
...
- Combine with chunking (see `hdf5_chunking`) and compression (see `hdf5_filters`)
...
- **Slow reads**: Increase chunk cache (see `hdf5_chunking`), check alignment
