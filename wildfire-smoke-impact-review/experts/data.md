...
parameters:
  ...
  continuation_contracts:
    ...
    - id: fire_to_geography
      ...
      next_action: call the bounding-box tool on the selected fire's perimeter (fire_perimeter.geojson) and emit workflow_state.region = its bbox
    - id: region_to_smoke
      when_state:
        region:
          exists: true
      ...
      next_action: query the NWS smoke forecast over the region bbox in prior workflow_state.region (use the actual numbers), save it to smoke_forecast.geojson, and emit workflow_state.acquisition.smoke_path
    - id: smoke_to_air
      when_state:
        acquisition.smoke_path:
          exists: true
      ...
      next_action: query AirNow monitors over the same region bbox in prior workflow_state.region (use the actual numbers), save it to air_quality.geojson, and emit workflow_state.acquisition.monitors_path
---
